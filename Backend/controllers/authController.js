...
export const register = (req, res) => {
  ...
  db.query(checkSql, [id], (err, results) => {
    ...
    if (results.length > 0) {
      return res
        .status(400)
        .json({ success: false, message: "ID / NIM sudah terdaftar!" });
    }
    ...
    db.query(insertWithPasswordSql, [id, name, email, password], (err) => {
      if (err) {
        // Fallback: coba insert tanpa kolom PASSWORD
        const fallbackSql = `
          INSERT INTO customers 
          (CUST_ID, CUST_NAME, EMAIL, ADDRESS, PLACE_OF_BIRTH, CONTACT_NUMBER, GENDER_ID, CREATED_AT, CREATED_BY) 
          VALUES (?, ?, ?, '-', '-', '-', 'L', NOW(), 'SYSTEM')
        `;
        db.query(fallbackSql, [id, name, email], (err2) => {
          if (err2) return res.status(500).json({ success: false, message: "Registrasi gagal: " + err2.message });
